...
public class SearchPriceController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<RabbitDTO> list2 = dao.getAllRabbit();
        try{
            float min = Float.parseFloat(min_raw);
            float max = Float.parseFloat(max_raw);
            list = dao.searchRabbitInRange(min, max);
        }catch(NumberFormatException e){
            System.out.println(e);
        }
        HttpSession session = request.getSession();
        session.setAttribute("data", list2);
        session.setAttribute("min", min_raw);
        session.setAttribute("max", max_raw);
